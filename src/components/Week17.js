...
/**
 * Week 17 — Momentum, Motivation & Revisit Your Why
 * - Autosaves to localStorage (key: "week17-notes")
 * - Weekly check-in + celebrate wins + revisit your “why”
 * - Daily journaling + gratitude + visualization + affirmations + 30-min walk reflection
 */

export default function Week17() {
  const STORAGE_KEY = "week17-notes";
  ...
  const Day = ({ name, title }) => (
    <div style={card}>
      ...
      <textarea style={ta} placeholder="How did the walk feel? Any shifts in mood or energy?" value={data[name].walk} onChange={e=>setField([name,"walk"], e.target.value)} />
    </div>
  );

  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
      <h1>Welcome to Week 17!</h1>
      <p>Keep your momentum, celebrate your wins, and reconnect with your core “why”.</p>
      ...
      <label>How did it feel to incorporate exercise/body movement? Any shifts in mood or energy?</label>
      <textarea
        style={taLong}
        placeholder="Reflect on your experience..."
        value={data.movementExperience}
        onChange={e=>setField(["movementExperience"], e.target.value)}
      />

      <label>We’ve made it to Week 17! What positive outcomes are you seeing from consistent effort?</label>
      <textarea
        style={taLong}
        placeholder="Celebrate your wins..."
        value={data.celebrateWins}
        onChange={e=>setField(["celebrateWins"], e.target.value)}
      />

      <h2>Revisiting Your “Why”</h2>
      <textarea
        style={taLong}
        placeholder="Which core values and vision are motivating you right now? How will you honor them this week?"
        value={data.revisitWhy}
        onChange={e=>setField(["revisitWhy"], e.target.value)}
      />
